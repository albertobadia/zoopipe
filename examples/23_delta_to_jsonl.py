...
from zoopipe import DeltaInputAdapter, JSONOutputAdapter, Pipe, PipeManager
...
def main():
    ...
    output_jsonl = os.path.join(base_dir, "examples/output_data/users_from_delta.jsonl")
    ...
    if not os.path.exists(table_location):
        print(f"Error: Delta table not found at {table_location}")
        print("Please run examples/22_csv_to_delta.py first.")
        return

    # Define the pipe
    pipe = Pipe(
        input_adapter=DeltaInputAdapter(table_location),
        output_adapter=JSONOutputAdapter(output_jsonl, format="jsonl"),
        schema_model=UserSchema,
    )

    print(f"Reading from Delta table: {table_location}")
