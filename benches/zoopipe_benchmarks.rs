...
fn bench_uuid_generation(c: &mut Criterion) {
    ...
    group.bench_function("uuid_v4_single", |b| b.iter(uuid::Uuid::new_v4));
...
fn bench_boxed_reader_seek(c: &mut Criterion) {
    ...
    group.bench_function("seek_start", |b| {
        let cursor = std::io::Cursor::new(data.clone());
        let mut reader = BoxedReader::Cursor(cursor);
        b.iter(|| reader.seek(SeekFrom::Start(black_box(50_000))).unwrap());
    });
    ...
    group.bench_function("seek_end", |b| {
        let cursor = std::io::Cursor::new(data.clone());
        let mut reader = BoxedReader::Cursor(cursor);
        b.iter(|| reader.seek(SeekFrom::End(black_box(-1000))).unwrap());
    });
...
fn bench_storage_controller(c: &mut Criterion) {
    ...
    group.bench_function("parse_local_path", |b| {
        b.iter(|| StorageController::new(black_box("/tmp/data/file.csv")).unwrap());
    });

    group.bench_function("parse_local_deep_path", |b| {
        b.iter(|| {
            StorageController::new(black_box("/very/deep/nested/path/to/some/file.parquet"))
                .unwrap()
        });
    });
...
fn bench_memory_stats(c: &mut Criterion) {
    ...
    group.bench_function("get_rss", |b| {
        b.iter(|| memory_stats::memory_stats().map(|m| m.physical_mem));
    });

    group.bench_function("get_process_ram_rss", |b| {
        b.iter(zoopipe_rust_core::pipeline::get_process_ram_rss);
    });
...
fn bench_pipe_counters(c: &mut Criterion) {
    ...
    group.bench_function("new", |b| b.iter(PipeCounters::new));
    ...
    group.bench_function("fetch_add_all_counters", |b| {
        ...
        b.iter(|| {
            counters
                .total_processed
                .fetch_add(black_box(100), Ordering::Relaxed);
            counters
                .success_count
                .fetch_add(black_box(95), Ordering::Relaxed);
            counters
                .error_count
                .fetch_add(black_box(5), Ordering::Relaxed);
            counters
                .batches_processed
                .fetch_add(black_box(1), Ordering::Relaxed)
        });
...
fn bench_error_creation(c: &mut Criterion) {
    ...
    group.bench_function("field_conversion", |b| {
        b.iter(|| PipeError::FieldConversion {
            field: black_box("user_id").to_string(),
        });
    });

    group.bench_function("invalid_config", |b| {
        b.iter(|| PipeError::InvalidConfig(black_box("missing batch_size").to_string()));
    });

    group.bench_function("other", |b| {
        b.iter(|| PipeError::Other(black_box("unexpected error").to_string()));
    });
...
fn bench_arrow_builders(c: &mut Criterion) {
    use arrow::array::{BooleanBuilder, Float64Builder, Int64Builder, StringBuilder};
...
fn bench_rayon_threadpool(c: &mut Criterion) {
    ...
    group.bench_function("par_iter_sum_10000", |b| {
        b.iter(|| data.par_iter().sum::<i64>());
    });

    group.bench_function("par_iter_map_10000", |b| {
        b.iter(|| data.par_iter().map(|x| x * 2).collect::<Vec<_>>());
    });
...
fn bench_crossbeam_channel(c: &mut Criterion) {
    ...
    group.bench_function("unbounded_create", |b| b.iter(unbounded::<i64>));
...
fn bench_bytes_operations(c: &mut Criterion) {
    ...
    group.bench_function("bytes_from_vec", |b| b.iter(|| Bytes::from(data.clone())));
    ...
    group.bench_function("bytes_clone", |b| b.iter(|| bytes.clone()));
