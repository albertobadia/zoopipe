...
def test_thread_executor(tmp_path):
    ...
    # Use 2 workers and small chunksize
    ...
    # Check that we actually used multiple threads
    thread_ids = {entry["metadata"]["thread_id"] for entry in output_data}
    # Ideally should be > 1, but scheduling is not guaranteed.
    # However, with 4 items and 0.1s sleep, single thread would take > 0.4s.
    # With 2 threads it should be faster (~0.2s + overhead).
