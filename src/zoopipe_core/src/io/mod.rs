...
pub fn get_reader(path: &str) -> std::io::Result<BoxedReader> {
    let boxed_reader = if storage::is_cloud_path(path) {
        let store_controller =
            storage::StorageController::new(path).map_err(std::io::Error::other)?;
        BoxedReader::Remote(RemoteReader::new(
            store_controller.store(),
            object_store::path::Path::from(store_controller.path()),
        ))
    } else {
    ...
    if path.ends_with(".gz") {
        ...
        Ok(BoxedReader::Gzip(Box::new(BufReader::new(GzDecoder::new(
            r,
        )))))
    } else if path.ends_with(".zst") {
        let r = SendBufReader(Box::new(BufReader::new(boxed_reader)));
        Ok(BoxedReader::Zstd(Box::new(BufReader::new(
            ZstdDecoder::new(r).map_err(std::io::Error::other)?,
        ))))
    } else {
        Ok(boxed_reader)
    }
...
pub fn get_writer(path: &str) -> std::io::Result<BoxedWriter> {
    ...
    if path.ends_with(".gz") {
        Ok(BoxedWriter::Gzip(Box::new(GzEncoder::new(
            boxed_writer,
            flate2::Compression::default(),
        ))))
    } else if path.ends_with(".zst") {
        Ok(BoxedWriter::Zstd(Box::new(
            ZstdEncoder::new(boxed_writer, 0).map_err(std::io::Error::other)?,
        )))
    } else {
        Ok(boxed_writer)
    }
...
impl<R: std::io::BufRead> std::io::BufRead for CountingReader<R> {
    ...
    fn consume(&mut self, amt: usize) {
        ...
        self.count
            .fetch_add(amt as u64, std::sync::atomic::Ordering::Relaxed);
    }
}
...
impl Seek for BoxedReader {
    fn seek(&mut self, pos: SeekFrom) -> std::io::Result<u64> {
        match self {
            ...
            BoxedReader::Gzip(_) | BoxedReader::Zstd(_) => {
                Err(std::io::Error::other("Cannot seek on compressed stream"))
            }
        }
    }
}
...
impl ChunkReader for BoxedReader {
    ...
    fn get_read(&self, start: u64) -> parquet::errors::Result<Self::T> {
        match self {
            ...
            BoxedReader::Cursor(c) => {
                let bytes = c.get_ref();
                Ok(BoxedReaderChild::Bytes(Bytes::copy_from_slice(
                    &bytes[start as usize..],
                )))
            }
            BoxedReader::Remote(r) => {
                let mut child_reader =
                    RemoteReader::new_with_len(r.store.clone(), r.path.clone(), r.file_len);
                child_reader
            ...
            BoxedReader::Gzip(_) | BoxedReader::Zstd(_) => {
                Err(parquet::errors::ParquetError::General(
                    "Cannot create ChunkReader for compressed stream".into(),
                ))
            }
        }
    }
    ...
    fn get_bytes(&self, start: u64, length: usize) -> parquet::errors::Result<Bytes> {
        match self {
            ...
            BoxedReader::Gzip(_) | BoxedReader::Zstd(_) => {
                Err(parquet::errors::ParquetError::General(
                    "Cannot read bytes from compressed stream".into(),
                ))
            }
        }
    }
...
impl Write for BoxedWriter {
    fn write(&mut self, buf: &[u8]) -> std::io::Result<usize> {
        match self {
            ...
            BoxedWriter::Closed => Err(std::io::Error::new(
...
mod tests {
    ...
    fn test_s3_compressed_roundtrip() {
        ...
        // 1. Write compressed data to "S3" (InMemory)
        {
            ...
            let mut writer = BoxedWriter::Gzip(Box::new(GzEncoder::new(
                boxed_writer,
                flate2::Compression::default(),
            )));
        ...
        // 2. Read compressed data from "S3"
        {
            ...
            let mut reader =
                BoxedReader::Gzip(Box::new(std::io::BufReader::new(super::GzDecoder::new(r))));
