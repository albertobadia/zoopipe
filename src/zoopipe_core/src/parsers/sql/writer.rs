...
use super::backends::{PostgresCopyBackend, GenericInsertBackend};
use super::utils::{init_drivers, ensure_parent_dir, is_postgres};
...
impl SQLWriter {
    ...
    pub fn new(
        ...
        match mode {
            "replace" | "append" | "fail" => (),
            _ => return Err(PyRuntimeError::new_err("mode must be 'replace', 'append', or 'fail'")),
        }
    ...
    pub fn write_batch(&self, py: Python<'_>, entries: Bound<'_, PyAny>) -> PyResult<()> {
        ...
        self.backend.write_batch(py, &all_records, &fields, &self.table_name)?;
...
impl SQLWriter {
    fn ensure_table_created(
        ...
        let fields = fieldnames.as_ref()
            .ok_or_else(|| PyRuntimeError::new_err("Fieldnames should be set by ensure_table_created()"))?;
